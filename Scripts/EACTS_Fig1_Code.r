#* Set up targeted and untargeted data for pathway analysis - impute missing untargeted values and join metadata and key with targeted data
  ...
  #+ Impute missing C18 values while accounting for PGD status and time associated with the sample ID
    ...
    combined_UFT <- HILIC_merged_data %>%
    ...
    for (i in pt_imp_id){
      ...
      rows_for_imp <- combined_UFT[(combined_UFT$Clinical_PGD == subset[1,]$Clinical_PGD) & (combined_UFT$Time == imp_time),]
      ...
      combined_UFT <- rbind(combined_UFT,imp_dat)
    }
...
#* Pathway analysis and metabolite trends for variations in time only
  #+ Perform linear mixed effects model for each metabolite - Fig 1A and Fig 2A (put results through MetaboAnalyst Functional Analysis to obtain final figures)
    # Log2 transform the data
    combined_UFT[,7:ncol(combined_UFT)] <- log2(combined_UFT[,7:ncol(combined_UFT)]+1)
    ...
    for (i in colnames(combined_UFT)[7:ncol(combined_UFT)]) {
      # Create a formula for the linear mixed effects model
      dat <- combined_UFT %>% select(all_of(i), Time, Patient, Clinical_PGD)
  ...
  #+ Linear mixed effects on time and time/PGD interaction on targeted data: plot time course of top 5 metabolites based on lowest pvals with fitted splines - Fig 1B, Fig 2B
    ...
    targ_zscores <- combined_TFT[,5:ncol(combined_TFT)] %>%
      mutate(across(everything(), ~ (.-mean(., na.rm = TRUE)) / sd(., na.rm = TRUE)))
    targ_zscores <- combined_TFT[,1:4] %>%
      cbind(targ_zscores)
  ...
  #+ Slopes of splines for top metabolites - Fig 1C
    ...
    ggplot(time_combined_slopes, aes(x = reorder(Identified_Name, Slope), y = Slope, fill = Slope > 0)) +
      ...
      theme_minimal()
#* Pathway analysis and metabolite trends for variations in time and PGD status
...
#* HCA with heatmaps, PLS-DA, and volcano plots
  ...
  #+ Volcano Plots
    ...
    volc_unt(fc_unt24, unt_24PGD_ttest, "24h") +
      ggtitle("Volcano Plot for PGD vs Non-PGD at 24h")
    ggsave(
      filename = "./Volcano_PGD_vs_NonPGD_24h.png",
      plot = last_plot(),
      device = 'png',
      width = 6,
      height = 4,
