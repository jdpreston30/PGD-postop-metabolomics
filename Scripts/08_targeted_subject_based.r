#+ 8.1: Fix imputed subjects to set them back to NAs
  #- 8.1.1: Create NA rows for missing samples
  ...
  #- 8.1.2: Drop any existing rows for those combos
  ...
  #- 8.1.3: Bind them back
      combined_TFT_LMM <- bind_rows(combined_TFT_LMM_i, new_rows) %>%
        arrange(Patient, Time)
#+ 8.2: limma orientation and full workflow
  #- 8.2.1: Ensure factors have the right baseline levels
  ...
  #- 8.2.2: Identify metabolite columns (4+)
      met_cols <- names(combined_TFT_LMM)[4:ncol(combined_TFT_LMM)]
  #- 8.2.3: Build expression matrix as features x samples (rows = metabolites, cols = samples)
  ...
  #- 8.2.4: Design matrix (rows must match number of columns in expr == nrow(combined_TFT_LMM))
      design <- model.matrix(~ Time * Clinical_PGD, data = combined_TFT_LMM)
  #- 8.2.5: Blocking factor for repeated measures (length must equal ncol(expr))
      block <- combined_TFT_LMM$Patient
  #- 8.2.6: Estimate within-subject correlation and fit
  ...
  #- 8.2.7: Extract Group x Time interaction results (flat vs non-flat between groups)
      tt_inter <- limma::topTable(fit, coef = "Time24:Clinical_PGDY", number = Inf, sort.by = "P")
  #- 8.2.8: Tidy tibble with metabolite, estimate, p, FDR
