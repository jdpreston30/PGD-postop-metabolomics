...
#+ 1.2: Prep to append asterisks onto targeted with isomers for targeted
...
#+ 1.3: Preprocess Untargeted Feature Tables into Tidy format
  ...
  #- 1.3.4: For HILIC
    HILIC_unt_proc <- process_feature_table(HILIC_unt_UP, sequence, "HILIC") %>%
      ...
      filter(!str_detect(Sample_ID, "(D0|S0)$")) %>%
      arrange(Sample_ID) 
  #- 1.3.5: Join together, add metadata
...
#+ 1.5: Transformations and QC
  ...
  #- 1.5.2: Run cleanup function on Untargeted
...
#+ 1.6: Final Assignments of feature tables and column names
  #- 1.6.1: Untargeted
